defmodule Rulestead.Credo.NoRawTraitsInLogger do
  use Credo.Check,
    base_priority: :high,
    category: :warning,
    explanations: [
      check: """
      Logger metadata must not carry raw trait-like keys such as email or IP.
      Redact before attaching metadata to logs.
      """
    ]
  ...
  @message "Logger metadata must not include raw trait keys. Redact before attaching metadata."
  ...
  defp traverse({{:., _, [{:__aliases__, _, [:Logger]}, :metadata]}, _, [meta_ast]} = ast, issues, issue_meta) do
    {ast, issues ++ issues_for_meta_ast(meta_ast, issue_meta)}
  end
  ...
  defp issues_for_meta_ast(meta_ast, issue_meta) when is_list(meta_ast) do
    Enum.flat_map(meta_ast, &issue_for_pair(&1, issue_meta))
  end
  ...
  defp issue_for_pair(_pair, _issue_meta), do: []
