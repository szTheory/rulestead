...
if Code.ensure_loaded?(Credo.Check) and Code.ensure_loaded?(Credo.SourceFile) do
  defmodule Rulestead.Credo.NoMutationOutsideMulti do
    ...
    defp rulestead_schema?({:__aliases__, _, [:Rulestead, module]})
         when module in @rulestead_modules, do: true

    defp rulestead_schema?({:__aliases__, _, [module]}) when module in @rulestead_modules,
      do: true
