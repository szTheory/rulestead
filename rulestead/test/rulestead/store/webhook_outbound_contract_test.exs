  ...
  defmacro webhook_outbound_contract_tests do
    quote do
      ...
      test "create and fetch destination parity", %{store: store} do
        command = Command.CreateWebhookDestination.new(%{
        ...
        {:ok, fetched} = store.fetch_webhook_destination(Command.FetchWebhookDestination.new(dest.id))
      ...
      test "list destinations parity", %{store: store} do
        ...
        {:ok, page} = store.list_webhook_destinations(Command.ListWebhookDestinations.new(environment_key: env))
