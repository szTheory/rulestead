...
| 119-01-02 | 01 | 1 | CIDX-01, CIDX-02 | T-119-02, T-119-03 | Required-check semantics distinguish documented intent, live state, advisory signals, and release posture | source assertion + live metadata evidence | Per-item loop asserts all 11 workflow files, all 8 `ci.yml` job IDs, `gh workflow list`, `required_status_checks`, documented-vs-live state, `openfeature-companion` absent from `release_gate.needs`, and exact `Branch not protected` output or concrete copied branch-protection API/status fields/error text. | yes | planned |
...
| 119-02-01 | 02 | 2 | CIDX-02 | T-119-05 | Timing claims are tied to run IDs or explicit fallback; p95 is not invented | source assertion + live metadata evidence | Per-item loop asserts `gh run list --repo szTheory/rulestead --workflow ci.yml --limit 20`, `gh run view`, events, wall-clock, longest job, critical path, duplicated work, runner/tool versions, Postgres, exact `p95 target unavailable from current sample` fallback or concrete numeric p95 pattern, and run ID or live-metadata failure. | yes | planned |
...
| 119-03-02 | 03 | 3 | CIDX-01, CIDX-02, CIDX-03 | T-119-11, T-119-12 | Browser/release/security recommendations preserve audit-only no-go guardrails | source assertion | Per-item loop asserts primary recommendation, Playwright trace/retry mismatch, D-18 exact strings `FleetDesk host-branded` and `no product UI, brand, or design-system expansion`, failure artifact/quarantine wording, official pattern notes, and every no-go guardrail. | yes | planned |
| 119-03-03 | 03 | 3 | CIDX-01, CIDX-02, CIDX-03 | T-119-10 | Source coverage prevents false completion claims and blocks behavior-changing diffs | source assertion + diff gate | Per-row assertions require GOAL, each CIDX-01..03 row, RESEARCH workflow inventory, RESEARCH timing baseline, RESEARCH Mix diagnostics, RESEARCH classification matrix, and every individual D-01..D-21 row to be marked `COVERED`; deferred rows must be `EXCLUDED - deferred by CONTEXT.md`; diff gate rejects `.github/workflows/`, `scripts/`, `rulestead/`, `rulestead_admin/`, `examples/`, `mix.exs`, `mix.lock`, and `playwright.config.ts` changes. | yes | planned |
