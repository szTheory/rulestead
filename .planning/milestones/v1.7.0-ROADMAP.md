...
`v1.7.0` closed the reusable-targeting safety arc by routing high-blast-radius protected-environment audience edits through governed change requests after v1.6 preview and dependency truth were proven. Core owns blast-radius threshold evaluation and change-request integration; mounted admin owns proposal, review, and policy-aware visibility inside the existing envelope.

## Milestone Summary

**Key Decisions:**

- Reuse the existing change-request envelope — no parallel governance path.
- Blast-radius evaluation consumes v1.6 preview fingerprints and dependency truth only; no observability-backed population counts.
- Frozen blast-radius evidence on change-request show; approve gate uses live dependency visibility tier.

**Issues Resolved:**

- Protected-environment audience mutations above configurable thresholds require governed approval instead of direct apply.
- High-blast-radius proposals submit, approve, and execute through the existing governed mutation path with stale-preview rejection.
- Mounted admin routes threshold breaches to proposal/review with calm operator copy and policy-aware partial visibility.
- `mix verify.phase60`, release-contract drift guards, and quickstart parity close support truth for blast-radius governance.

**Issues Deferred:**

- Automatic guarded rollout advancement (`ROL-04`) — queued for v1.8.0.
- Richer host-supplied preview evidence (`IMP-05`).
- Draft-only targeting presets (`ADM-05`).

**Technical Debt Incurred:**

- No release-blocking technical debt accepted at close; optional CI proof scope (`blast_radius_governance`) remains opt-in alongside default CI.
