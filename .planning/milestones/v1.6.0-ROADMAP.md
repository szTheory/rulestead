...
`v1.6.0` deepened already-shipped reusable audience targeting with impact previews, dependency visibility, explainability, mounted operator workflows, and support truth while preserving deterministic snapshots and the linked sibling-package release model. Core owns preview semantics, dependency inventory, validation, and fail-closed blockers; mounted admin renders policy-aware workflows inside the existing envelope.

## Milestone Summary

**Key Decisions:**

- Activate `v1.6.0` as reusable targeting deepening instead of adding a new targeting primitive.
- Keep reusable targeting previews authored-state and explicit-sample based.
- Preserve linked-version two-package release model: `rulestead` owns contracts and validation; `rulestead_admin` owns mounted presentation.

**Issues Resolved:**

- Operators can preview, token-confirm, and audit reusable audience mutations without false precision or runtime lookup drift.
- One core dependency truth now blocks unsafe publish, archive/delete, promotion, compare, replay, and manifest paths.
- Mounted admin surfaces make audience dependencies, previews, confirmations, and explain traces visible inside the existing policy envelope.
- Proof, docs, and release-contract truth describe the same bounded reusable-targeting scope.

**Issues Deferred:**

- Richer host-supplied impression or cohort previews (`IMP-05`).
- Optional draft-only targeting presets (`ADM-05`).
- Blast-radius-threshold approvals for protected audience edits (`GOV-01`).
- Automatic guarded rollout advancement windows (`ROL-04`).

**Technical Debt Incurred:**

- No release-blocking technical debt was accepted at close; optional CI proof scope (`reusable_targeting_deepening`) remains opt-in alongside default CI.

---

# Roadmap: Rulestead (archived at close)
