...
created: 2026-06-14
relocated: 2026-06-17
relocated_from: .planning/phases/115-foundations-hardening/115-FOUNDATIONS-CONTRACT.md
relocated_reason: >
  Moved to brandbook/ (a living, non-archived home) so scripts/check_admin_foundations.py
  keeps validating admin CSS drift after the v1.17 phase directory was archived at the
  v1.18 milestone kickoff (commit b78bedd). Content unchanged; only the location moved.
---

# Admin Foundations Contract (originally Phase 115)
...
and `brandbook/brand-book.md`. It is enforced by `scripts/check_admin_foundations.py`
(run in `scripts/ci/lint.sh`).
